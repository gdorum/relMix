...
vignette: >
  %\VignetteIndexEntry{relMix 2.0}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

This vignette gives an introduction to RelMix 2.0, the graphical user interface of the relMix package. We will present the main features of the GUI with two examples.
...
The first step in RelMix is to import data. The import files must be either comma, semicolon or tab-separated and must have dots as decimal separators. Examples will be given for the three types of files that are needed: mixture profile, reference profiles and allele frequencies. The mixture profile and reference profiles should contain the same loci in the same order, while the allele frequency file can contain additional loci not in the profiles.
...
The last step is to specify dropout and drop-in values. By default all values are set to 0. In this case we expect no dropout from the mother so we set her dropout probability to 0, while the child is given a dropout probability of 0.1. The drop-in value is set to 0.05 (Figure 6).
...
###3. Pedigree
Since the hypotheses in this example are not covered by the built-in pedigrees, we will provide RelMix with a custom pedigree through an R script. This R script can for instance be exported from Familias 3, but can also be generated manually. The script below shows the code needed to provide the first pedigree where the two contributors are mother and child. **Note:** 1) the pedigree must be defined in a variable called "ped", and 2) the names of the individuals in the pedigree (the id) must correspond to the names of individuals in the reference profiles.
```{r, eval=FALSE}
persons <- c("C2","C1")
...
```{r, out.width =  350, fig.retina = NULL,echo=FALSE,fig.cap="Figure 14: Computed LR for example 2."}
